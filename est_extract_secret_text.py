...
def est_bit_proportion(byte_array1, byte_array2):
    bp = 0
    for i in range(len(byte_array1)):
        if byte_array1[i] == 1:
            bp += i / len(byte_array1[i])
    for i in range(len(byte_array2)):
        if byte_array2[i] == 1:
            bp += i / len(byte_array2[i])
    return bp
...
def est_extract():
    ...
    for i in range(1, 4):
        for j in range(1, 4):
            bp = est_bit_proportion(bytearray(i), bytearray(j))
            print('p=', i, 'q=', j, 'bp=', '%.3f'%bp)
            if bp == 0.5: 
                p = i
                q = j
