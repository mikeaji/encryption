...
def est_bit_proportion(byte_array1):
    bp = 0
    for i in range(len(byte_array1)):
        if byte_array1[i] == 1:
            bp += 1
    return bp / len(byte_array1)
...
def est_extract():
    ...
    for i in range(3):
        for j in range(3):
            ...
            bp = float(str(est_bit_proportion (message_bit_array))[:5]) #(bytearray(i), bytearray(j))
            #print(" p = " + str(i) + " " + " q = " + str(j) + " " +" bp= " + str("%.3f" %bp))
            print('p='+str(p),'q='+str(q),'bp='+str(bp))
            if bp < 0.5:
